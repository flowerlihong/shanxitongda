                        ...
                        <div class="card facebook-card animated fadeInUp" >
                            ...
                            <div class="card-header no-border">
                                <div class="col-xs-2 facebook-avatar" style="padding:0px;">
                                    ...
                                    <div v-if="item.procDefName.length>4" class=" title5">
                                        <p> {{item.procDefName.substring(0,7)}}</p>
                                    </div>
                                    <div v-if="item.procDefName.procDefName<=4" class=" title4">
                                        <p> {{item.procDefName}}</p>
                                    </div>
    
                                </div>
                                <div class="facebook-date head-ft16">
                                    <span class="c999 ft14">申请人：{{item.startUserName}}</span>
                                </div>
...
export default {
    ...
    methods: {
        operate(type,item){
            ...
            if(type== 3){
                //设计
                var param =  { modelId: item.id,key:item.key,formKey:item.formKey,category: item.category,name:encodeURI(item.name)};
        ...
        onScrollBottom() {
            ...
            this.util.post('/workflow/findMyApprovalUnFinshed?pageSize=' + _this.pageSize, param, function (res) {
                //_this.util.unmask();
                if ((_this.util.isNotEmpty(_this.search) || _this.advancedQuery.enable) && type == undefined) {
                    _this.util.unmask();
                }
