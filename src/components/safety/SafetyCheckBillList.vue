<template>
  <div class="choose">
      <div class="row" style="margin:0;">
          <search  v-model="search" placeholder="按项目搜索" :autoFixed="false"  @on-submit="handleClick" ref="search"></search>
          ...
          <scroller height="-60" :use-pulldown="true" :pulldown-config="pulldown" :use-pullup="true" :pullup-config="pullup" lock-x @on-pulldown-loading="loadTop" @on-pullup-loading="onScrollBottom" ref="scrollerBottom">
            <div>
              <div v-for="item in checkBillDatas">
                <div class="card facebook-card" @click="showCheckBill(item)">
                   ...
                   <div class="card-header no-border">
                      ...
                      <div class="facebook-date">
                          ...
                          <span style="color: #101001;font-size: 14px;">{{item.project == undefined?'':(item.project.length>20?item.project.substr(0,20)+'...':item.project)}}</span>
                      </div>
                      <div class="facebook-date">
                          <span class="c999">检查项:</span>
                          <span style="color: #101001;font-size: 14px;">{{item.checkItem}}</span>
                      </div>
                      <div class="facebook-date">
                          <span class=" c999 title-item" style="width:53%;">检查人:{{item.checker.length>15? item.checker.substring(0,15)+'..':item.checker}}</span>
    ...
    <div class="right-panel">
        <ul>
            <li>
                <a class="mod-li-icon" @click="addSafetyCheckBill">
...
export default {
  ...
  watch:{
      temp:function(newVal){
          if(newVal.indexOf("loadSafetyBillList")>=0){
              this.loadTop();
          }
  ...
  methods: {
    ...
    loadData(type, creatTime) {
        var param = {
            ...
            operateType: 'getSafetyBillLists',
    ...
    showCheckBill(item){
        this.$router.push({name:'safetyCheckBillShow',query:{id:item.id}});
    },
    
  },
  components:{
    Search,Scroller, LoadMore
  }
}
</script>
