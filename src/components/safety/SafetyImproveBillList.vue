...
export default {
  ...
  watch:{
      temp:function(newVal){
          if(newVal.indexOf("loadSafetyImproveList")>=0){
              this.loadTop();
          }
  ...
  activated(){
        ...
        if (this.improveBillDatas.length == 0||refresh) {
  ...
  methods: {
    ...
    loadData(type, creatTime) {
        var param = {
            ...
            operateType: 'getSafetyImproveLists',
    ...
    showImproveBill(item){
        this.$router.push({name:'safetyImproveBillShow',query:{id:item.id,dataType:'safety'}});
