<template>
    <div class="choose">
        ...
        <div >            <!-- class="vux-1px-t" -->
            <swipeout>
                <div v-for="(item,index) in safetyData" :key="item.Id">
                                ...
                                <div class="card-header no-border">
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            检查内容:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.sCheckContent == undefined?'':(item.sCheckContent.length>35?item.sCheckContent.substr(0,35)+'...':item.sCheckContent)}}
                                            </span>                                           
                                        </span>                                      
                                    </div>                                   
                                    <div class="facebook-date">
                                        ...
                                        <span class=" c999 title-item infoItems-right" >
                                            检查结果:
                                            <span :class="{'color-content':true,'color-red':item.sCheckResult=='不合格'}">
                                                {{item.sCheckResult}}
                                            </span>
                                        </span>
                                    </div>                                    
...
export default {
  ...
  methods:{
      ...
      showDetail(index){
          ...
          this.$router.push({name:'addSafety',query:{pType:'detail',Id:_this.safetyData[index].Id}})
      },
      ...
      //alert('on button click ' + type)
        ...
        if(type=='delete'){
            ...
            this.util.post(url,params,function(res){
                ...
                _this.safetyData.splice(index,1)
            })
        }
      },
      handleEvents (type) {
    ...
    querryData(type,start){
     ...
     // obj['projectNumber'] = 
      ...
      var sql = "select a.* from U010_Security_Inspection a left join U_Project b on a.xmid=b.Id where "+  
      ...
      this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_WeChar_Data_Query&appId=WeChat',params,function(res){
        console.log(res)
        _this.safetyData=type ==1? _this.safetyData.concat(res.data):cloneObj(res.data);
    ...
    addBx(){
        this.$router.push({name:'addSafety',query:{pType:'addNew',Id:''}})
    }
