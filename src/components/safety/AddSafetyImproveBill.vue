...
import { Checker, CheckerItem,XButton,XTextarea,Datetime} from 'vux'
import SelectUser from '../common/SelectUser.vue'
export default {
  name: 'approval',
  components:{
      // 名字
      Checker, CheckerItem,SelectUser,XButton,XTextarea,Datetime
  },
  data () {
    return {
		left: {textAlign:'left'},
  ...
  methods: {
		...
		saveData(){
			...
			this.util.post("/admin/appDefaultFormAction!saveData.action",{mainFormData:JSON.stringify(mainData),businessKey:_this.checkId,
				sqlTableName:"T_SAFETY_CHECK",id:_this.checkId},function(result){
					if(result.success){
						_this.util.unmask();
						_this.sendMsg(_this.improverModel.userUid,result.msg,"safetyImproveBillShow");
						// _this.$router.push({name:'safetyImproveBillList',query:{refresh:true}});
		...
		sendMsg(sendMsgUsersIds,businessId,state){
			...
			var param = {
					...
					title:"["+this.checkBill.project+"]安全整改通知",
					description:"您有一条新的安全整改需要处理，点击查看",
