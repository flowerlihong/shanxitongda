<template>
	...
	<div class="movebx">
		<div class="price" style="text-align:left;background-color: #eeefef;">
			<span class="col-xs-3">{{model.title==undefined?'详情描述':model.title}}<span v-if="model.allowBlank!=undefined && model.allowBlank == false" class="span_required">*</span></span>
			<span class="col-xs-9" style="text-align:right;">
				<!-- <el-button v-if="!model.isReadOnly" size="small" @click="record(0)" type="primary">语音录入</el-button> -->
		...
		<mt-popup v-model="recordVisable" popup-transition="popup-fade" style="width:90%;padding-top:10px;height:6.3rem;text-align: center;">
			...
			<div>
				<img width="100" :src="baseUrl+'/static/img/record.jpg'">
			</div>
			<div>
				<h3>{{recordContent}}</h3>
			</div>
			<p style="margin: 0 0 0 10px;">
				<!-- <el-button @click="record(2)" style="width:48%;margin:0;height:1.3rem" type="default">取消</el-button>
				<el-button @click="record(1)" style="width:48%;margin:0;height:1.3rem" type="success">说完了</el-button> -->
					...
					success: function(res) {
						...
						_this.wx.uploadVoice({
							...
							success: function(res) {
								...
								_this.util.post("/attachment/voiceRecognition", { serverId: serverId, suite_id: window.suite_id, corp_id: window.corp_id }, function(res) {
