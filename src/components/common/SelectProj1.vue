<template>
  <div class="choose">
    ...
    <div :style="checkStyle">已选择:&nbsp;&nbsp;&nbsp;{{ msg }}</div>
    <!-- v-if="checkSee"-->
    <div id="scroller">
      <scroller
        height="-100"
        ...
        <div class="box2" style="margin-top:5px;text-align: left;">
          ...
          >
            ...
            <icon
              ...
              style="font-size:16px;"
            ></icon>
            <span class="proTxt">{{ item.lxmc }}</span>
          </li>
          <div
    ...
    msg: {
      get() {
        return this.projectList.length == 0 || this.radio == 0
          ? ''
          : this.projectList[this.radio - 1].lxmc
      }
    }
  },
  props: ['mess'],
  ...
  methods: {
    ...
    loadData(type, creatTime) {
      ...
      var params = {
        ...
        firstRows: _this.totalIndex + 1
      }
      var url =
        '/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_exercise&appId=WeChat&Type=getpracticeType'
    ...
    goback(radio, $event) {
      ...
      if (radio == null || radio == 0) {
        ...
        if (this.projectList[radio - 1].lxbh == 'ct') {
          this.mess.sysl = this.projectList[radio - 1].ctsysl
        } else {
          this.mess.sysl = this.projectList[radio - 1].sysl
        }
        this.mess.lx = this.projectList[radio - 1].lxmc
        this.mess.lxbh = this.projectList[radio - 1].lxbh
        this.mess.lxszid = this.projectList[radio - 1].lxszid

        this.mess.selectProjVisible = false
        // console.log(this.projectList[radio - 1])
        // console.log(this.projectList)
        this.$emit('selectProjCallback', {
          sysl:this.mess.sysl,
          lx: this.projectList[radio - 1].lxmc,
          lxbh: this.projectList[radio - 1].lxbh,
          lxszid: this.projectList[radio - 1].lxszid
          // projectTypeName:this.projectList[radio-1].projectTypeName ,//一下三项为国城进度模块所需字段
          // proManagerId:this.projectList[radio-1].proManagerId ,
          // projectTypeId:this.projectList[radio-1].projectTypeId ,
        })
      }
    },
    goback3() {
