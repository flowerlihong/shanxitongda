			...
			<scroller height="-100" :use-pulldown="true" style="padding-top: 5px;" 
				...
				<div class="box2" style="margin-top:5px;text-align: left;">	
					<li class="itemLi" 
						...
						<span class="proTxt">{{item.projectName}}</span>
					</li>
...
export default {
	...
	computed: {
		msg:{
			get(){
				return (this.projectList.length == 0||this.radio == 0)?'':this.projectList[this.radio - 1].projectName;
			}
		}
	},
	props: ['mess'],
	...
	methods: {
		...
		loadData(type, creatTime) {
			...
			_this.onFetching = true;
				...
				var sql="select a.*,b.UserName from U_Project a	left join ORG_User b on "+
			...
			this.util.post(url,params,function(res){
				...
				if (type == 'downLoad') {
					...
					if (res.data.length < _this.pageSize) {						
						_this.isEnd = true;
						_this.totalIndex+=res.data.length;
		...
		goback( radio, $event) {
			...
			if (radio == null || radio ==0) {
				...
				this.mess.projName = this.projectList[radio - 1].projectName;
				console.log(this.projectList[radio - 1])
				this.mess.projUid = this.projectList[radio - 1].xmid;
				this.$emit('selectProjCallback', 
					{ projectName: this.projectList[radio - 1].projectName, 
					xmid: this.projectList[radio - 1].Id,
					projectNum:this.projectList[radio-1].projectNum 
					});
			}
		},
		goback3() {
