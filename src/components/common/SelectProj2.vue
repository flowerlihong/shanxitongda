<template>
	<div class="choose">
		...
		<div :style="checkStyle" >已选择:&nbsp;&nbsp;&nbsp;{{msg.length>20?msg.substr(0,20)+'...':msg}}</div><!-- v-if="checkSee"-->
		<div id="scroller">
			<scroller height="-100" :use-pulldown="true" style="padding-top: 5px;" 
				...
				<div class="box2" style="margin-top:5px;text-align: left;">	
					<li class="itemLi" 
						@click="checkItem($event,index,item)" v-for="(item,index) in projectList" :key="index" >
	...
	computed: {
		msg:{
			get(){
				...
				return (this.projectList.length == 0||this.radio == 0)?'':this.projectList[this.radio - 1][this.showWord];
			}
		}
	},
	props: ['mess','showWord','theUrl'],
	created() {
		var _this = this;
		this.search = '';
	...
	methods: {
		...
		loadData(type, creatTime) {
			...
			_this.onFetching = true;
				...
				if(this.mess.moduleType && this.mess.moduleType == "gcPro"){
					sql = "select a.*,b.UserName from U_Project a left join ORG_User b "+
						  "on a.ProManagerId=b.Id where a.states='1' and a.lock='-5' and "+
						  " (RIGHT('"+this.departId+"',6)  = RIGHT(a.subCompany,6) or '"+this.departId+"' "+
						  " in(select DepartId from ORG_Depart where DepartType='dept')  or   CHARINDEX('"+this.departId+"',a.subCompany)>0)  and "+
						  " ISNULL(a.showWord,'') like '%"+this.search+"%' and isnull(b.UserName,'') like '%%'  "
				}else if(this.mess.moduleType&& this.mess.moduleType == "progressProj"){
					sql="select a.*, b.UserName, p.projectTypeId, p.projectTypeName "+
						"from U_Project a "+
						...
						"and  (RIGHT('"+this.departId+"',6)  = RIGHT(a.subCompany,6) or '"+this.departId+"'  "+
						"in(select DepartId from ORG_Depart where DepartType='dept')  or   CHARINDEX('"+this.departId+"',a.subCompany)>0) "
				}else{
					sql="select a.*,b.UserName from U_Project a	left join ORG_User b on "+
						"a.ProManagerId=b.Id where a.states='1' and a.lock='-5' and "+
						"('"+_this.userId+"' in (select UserId from ORG_User_Role where RoleId in "+
						" ('Role_0001_002','admin,''Role_Sys_Company_Admin')) or '"+_this.userId+"' in (select d.userIdListId from "+
		...
		goback( radio, $event) {
			...
			if (radio == null || radio ==0) {
				...
				//console.log(this.projectList[radio - 1])
