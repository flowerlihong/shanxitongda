	...
	<div id="mainForm" class="movebx" v-loading="loading" element-loading-text="拼命加载中" style="height: 100%;padding-bottom: 67px;width: 100%;" ref="mainForm">
		<div style="height: 100%;overflow:auto;width: 100%;">
			<div style="">
				...
				<div id="msg" class="price" style="text-align:left;border-top: 0px;height: 49px;">
				...
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">岗位<span class="span_required">*</span></span>
					<input :readonly="isReadOnly" v-model="mainData.POSITION_" class="col-xs-9" type="text">
				</div>
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">入职日期<span class="span_required">*</span></span>
					<datetime class="col-xs-9" :readonly="isReadOnly" v-model="mainData.ENTRY_DATE_" style="padding:0px;padding-right:10px;" default-selected-value="date" format="YYYY-MM-DD" :placeholder="'请选择日期'"></datetime>

				</div>
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">试用期<span class="span_required">*</span></span>
					
					<input :readonly="isReadOnly" v-model="mainData.PROBATION_PERIOD_" class="col-xs-9" type="text">
				</div>
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">薪资<span class="span_required">*</span></span>
					<input :readonly="isReadOnly" v-model="mainData.SALARY_" class="col-xs-9" type="number">
				</div>
...
export default {
	...
	data() {
		return {
			...
			mainData: {
				...
				formKey: 'apply_regularWork',
			},
			formKey: 'apply_regularWork',
			sqlTableName: 'T_REGULAR_WORK',
			modelName: '转正申请',
	...
	methods: {
		...
		cancle() {
			...
			if (!this.validUtil.isNotEmpty(this.mainData.ENTRY_DATE_)){ 
				this.util.failueToast("入职日期不能为空！");
				return false;
			}
			if (!this.validUtil.isNotEmpty(this.mainData.POSITION_)){ 
				this.util.failueToast("岗位不能为空！");
				return false;
			}
			if (!this.validUtil.isNotEmpty(this.mainData.PROBATION_PERIOD_)){ 
				this.util.failueToast("试用期不能为空！");
				return false;
			}
			if (!this.validUtil.isNotEmpty(this.mainData.SALARY_)){ 
				this.util.failueToast("薪资不能为空！");
				return false;
			}else{
				if(this.mainData.SALARY_ == 0){
					this.util.failueToast("薪资不能为零！");
				}
			}
			if (!this.validUtil.isNotEmpty(this.voiceRecognition.content)){ 
				this.util.failueToast("自我评价不能为空！");
				return false;
			}
			...
			this.mainData.SUBJECT_ = this.date + " " + this.sessionUtil.getUserName(this) + "的转正申请";
