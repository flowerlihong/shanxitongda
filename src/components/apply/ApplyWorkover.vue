	...
	<div id="mainForm" class="movebx" v-loading="loading" element-loading-text="拼命加载中" style="height: 100%;padding-bottom: 67px;width: 100%;" ref="mainForm">
		<div style="height: 100%;overflow:auto;width: 100%;">
			<div style="">
				...
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">开始时间<span class="span_required">*</span></span>
					<datetime class="col-xs-9" :readonly="isReadOnly" v-model="mainData.START_TIME_" style="padding:0px;padding-right:10px;" default-selected-value="date" format="YYYY-MM-DD HH:mm" :placeholder="'请选择日期'"></datetime>

				</div>
				<div class="price" style="text-align:left;">
					<span class="col-xs-3">结束时间<span class="span_required">*</span></span>
					<datetime class="col-xs-9" :readonly="isReadOnly" v-model="mainData.END_TIME_" style="padding:0px;padding-right:10px;" default-selected-value="date" format="YYYY-MM-DD HH:mm" :placeholder="'请选择日期'"></datetime>
...
export default {
	...
	data() {
		return {
			...
			mainData: {
				...
				formKey: 'apply_workover',
			},
			formKey: 'apply_workover',
			sqlTableName: 'T_WORK_OVER',
			modelName: '加班申请',
	...
	methods: {
		...
		cancle() {
			...
			if (this.validUtil.isNotEmpty(this.mainData.START_TIME_) && this.validUtil.isNotEmpty(this.mainData.END_TIME_)) {
				if (this.mainData.END_TIME_ < this.mainData.START_TIME_) {
					this.util.failueToast("结束时间不能比开始时间小！");
				return false;
				}
			}else{
				this.util.failueToast("时间不能为空！");
				return false;
			}
			...
			this.mainData.SUBJECT_ = this.date + " " + this.sessionUtil.getUserName(this) + "的加班申请";
