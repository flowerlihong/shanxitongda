		...
		<div style="height: 100%;overflow:auto;width: 100%;">
			<div style="">
				...
				<div class="price" style="text-align:left;border-top: 1px solid #dcdcdc;">
					<span class="col-xs-3">支付日期<span class="span_required">*</span></span>
					<datetime class="col-xs-9" :readonly="isReadOnly" v-model="mainData.DATE_" style="padding:0px;padding-right:10px;" default-selected-value="date" format="YYYY-MM-DD" :placeholder="'请选择日期'"></datetime>

				</div>

				<div class="price" style="text-align:left;padding-left:10px;">
					<div style="height: 35px;padding-right:10px;">
						<span class="col-xs-3" style="text-align:left;padding-left:0px;">
							付款方式<span class="span_required">*</span>
						</span>
						<span class="col-xs-9" style="padding-right: 0px;">
							<popup-radio :readonly="isReadOnly" :options="options" v-model="mainData.MODE_" style="padding-top: 0px;padding-bottom: 0px;padding-right: 0px;"></popup-radio>
						</span>
					</div>
				</div>

				<div class="price" style="text-align:left;">
					<span class="col-xs-3">开户行</span>
					<input :readonly="isReadOnly" v-model="mainData.BANK_" class="col-xs-9" type="text">
				</div>

				<div class="price" style="text-align:left;">
					<span class="col-xs-3">银行账户</span>
					<input :readonly="isReadOnly" v-model="mainData.BANK_ACCOUNT_" class="col-xs-9" type="text">
				</div>
				...
				<template v-for="(child,index) in children">
					<div @click="show(child)" v-if="(child.show == undefined || !child.show)" :key="index" class="middle" style="border-bottom: 1px solid #ccc;">
						<div class="pop" style="background-color: #eeefef;">
							<span class="col-xs-3" style="text-align:left;">付款明细{{index+1}}</span>
					...
					<div v-if="child.show" :key="index" class="middle">
						<div class="pop" style="background-color: #eeefef;">
							<span class="col-xs-4">付款明细{{index+1}}:</span>
						...
						<div class="price" style="text-align:left;">
							<span class="col-xs-3">名称</span>
							<input :readonly="isReadOnly" v-model="child.SUBJECT_" class="col-xs-9" type="text">
						</div>

						<div class="price" style="text-align:left;">
							<span class="col-xs-3">金额</span>
							<input :readonly="isReadOnly" v-model="child.AMOUNT_" class="col-xs-9" type="number">
						</div>
				...
				<div class="foot" style="padding-top:0;text-align:center;">
					...
					<div>
						<p class="footColor">共
							<span>{{children.length}}</span> 条明细，总付款金额(元):{{AMOUNT_}}</span>
						</p>
					</div>
				</div>
...
import { XTextarea, Divider, PopupRadio } from 'vux'
...
export default {
	...
	data() {
		return {
			...
			mainData: {
				...
				formKey: 'apply_payment',
			},
			formKey: 'apply_payment',
			sqlTableName: 'T_PAYMENT',
			modelName: '付款申请',
	...
	components: {
		// 名字
		XTextarea, Datetime, SelectProj, ProcessForm, selectQuarters, PopupRadio, Attachment, VoiceRecognition
	},
	computed: {
		...mapState({
		...
		AMOUNT_: function() {
			...
			for (var i = 0; i < this.children.length; i++) {
				...
				if (this.util.isNotEmpty(item.AMOUNT_)) {
					amount = amount + Number(item.AMOUNT_);
				}
			}
			this.mainData.AMOUNT_ = amount;
			return amount;
		}
	...
	methods: {
		...
		cancle() {
			...
			this.mainData.SUBJECT_ = this.date + " " + this.sessionUtil.getUserName(this) + "的付款申请";
			this.mainData.CONTENT_ = this.voiceRecognition.content;
			var subForm = { subFormId: 'subForm1', subFormForeignKey: 'PARENT_UID_', subFormTable: 'T_PAYMENT_DET' };
			//console.info("--------222------");
