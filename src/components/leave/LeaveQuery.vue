<template>
    <div class="choose">
        <div class="head">
            <search  v-model.trim="search" placeholder="搜索请假原因" :autoFixed="false"  @on-submit="searchFunc()" ref="search">
                </search>
        </div>
        ...
        <div >            <!-- class="vux-1px-t" -->
            <swipeout>
                <div v-for="(item,index) in leaveData" :key="item.Id">
                            ...
                            <div class="card facebook-card animated fadeInUp" @click="showDetail(index)">
                                ...
                                <div class="card-header no-border">
                                   ...
                                   <!--  <div class="facebook-date head-ft16">
                                        <span class=" ft14" style="color:#8994d2">请假类型{{item.leaveType}}</span>
                                    </div> -->
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item " > 
                                            ...
                                            <span class="color-content" style="display:block;float:right">
                                                {{item.leaveStartTime}} 至 <br>
                                                {{item.leaveEndTime}}
                                            </span>
                                            <!-- <span class="color-content">{{item.leaveStartTime}}</span> 
                                           至 <br>
                                            <span class="color-content" style="text-indent">{{item.leaveEndTime}}</span>  -->
                                        </span>
                                    ...
                                    <!-- <div class="facebook-date">
                                        <span class=" c999 title-item " > 
                                            ...
                                            <span class="color-content " >
                                                {{item.leaveReson==undefined?'':item.leaveReson.length>14?(item.leaveReson.substr(0,14)+'...'):item.leaveReson}}
                                                </span> 
                                ...
                                <div class="facebook-date">
                                        ...
                                        <span class=" c999 title-item infoItems-left" >请假天数:<span class="color-content">{{item.leaveDays}}</span></span>
                                        <span class=" c999 title-item infoItems-right" >请假类型:<span class="color-content">{{item.leaveType}}</span></span>
                                    ...
                                    <!-- <div class="facebook-date">
                                        ...
                                        <span class=" c999 title-item infoItems-right" style="width:37%;">申请人:<span class="color-content">{{item.leavePerson}}</span></span>
...
export default {
  ...
  methods: {
    ...
    showDetail(index) {
      var _this = this;
      _this.leaveData[index].pType = "detail";
      var isReadOnly = _this.leaveData[index].lock == null ? "false" : "true";
     
      this.$router.push({
        name: "addLeave",
        query: {
          pType: "detail",
          Id: _this.leaveData[index].Id,
          id: _this.leaveData[index].Id,
          _procInstId: _this.leaveData[index].ProcInsId,
    ...
    onButtonClick(type, Id, index) {
      ...
      if (type == "delete") {
        ...
        this.util.post(url, params, function(res) {
          ...
          _this.leaveData.splice(index, 1);
        });
      }
    },
    handleEvents(type) {
    ...
    querryData(type, start) {
      ...
      var searchParams = {
        leaveReson: _this.search
      };

      /* var sql = "select t.*,isnull(u.projectName,'') projectName,isnull(u.projectNum,'') projectNumber,"
                ...
                +"left join U012_Account_Manager a on t.accountId = a.Id" */
      ...
      this.util.post(
        ...
        function(res) {
          _this.util.unmask();
          _this.leaveData =
            type == 1 ? _this.leaveData.concat(res.data) : cloneObj(res.data);
