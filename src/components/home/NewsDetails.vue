  ...
  <div class="maimForm">
	  <div class="wrap">
		  	<div class="weui-cell  myCell" style="padding-left: 10px;border-top:1px solid #eee">
				<div class="weui-cell__hd"><span>标题</span></div>
            ...
            <div class="weui-cell  myCell" style="padding-left: 10px;border-top:1px solid #eee">
				<div class="weui-cell__hd"><span>类型</span></div>				
				<div class="weui-cell__bd" >					
					<span >{{newsDetails.type}}</span>
				</div>
			</div>
            <div class="weui-cell  myCell" style="padding-left: 10px;border-top:1px solid #eee">
				<div class="weui-cell__hd"><span>发文日期</span></div>				
				<div class="weui-cell__bd" >					
			...
			<div  style="text-align:left;">
				<x-textarea  :max="2000" style="width: 100%;"  :readonly="true"
							 placeholder="请输入备注..."	v-model="newsDetails.content"
	  ...
	  <div class="btns">
		  <p style="  margin: 0px;padding-bottom: 15px;   
				...
				<button  style="width:98%;height: 45px;line-height: 45px;border: 1px solid #ccc;"
						@click="cancelCallback()" class="weui-btn weui-btn_mini weui-btn_default">返回</button>
...
export default {
  components: {
	  Datetime,PopupPicker,VoiceRecognition,Attachment,ProcessMonitor1,XButton, XTextarea
  },
  data(){
	  return{
          newsDetails:{
      ...
      if(type == "发文管理"){
          /* var sql = "select issueDate as fwDate,title,hostUnit as depart,textContent as content,"+
           ...
           /*  var params ={
			...
			this.util.post("/admin/appAgentAction!runAgent.action?agentId=Agent_GCKG_Wechat_xiaoxi&appId=WeChat",{
				Type:"management",
				Id:_this.Id,
				userId:_this.userId,
         ...
         /* var sql = "select dispatchDate as fwDate,title,dispatchDepart as depart,dispatchcontent as content,"+
			...
			this.util.post("/admin/appAgentAction!runAgent.action?agentId=Agent_GCKG_Wechat_xiaoxi&appId=WeChat",{
				...
				if(res && res.status == true){
					_this.newsDetails.title = res.data.title	
					_this.newsDetails.type = res.name
					_this.newsDetails.fwDate = res.data.dispatchDate
					_this.newsDetails.content = res.data.dispatchcontent
					_this.newsDetails.depart = res.data.dispatchDepart	
				}
			})
      }
