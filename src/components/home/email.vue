            ...
            <div v-for="(item,index) in newsData" :key="index">
                <div class="news-card" @click="showDetail(index)">
                    ...
                    <div class="news-type c999">
                        <span>发件人：</span><span :class="item.type =='发文管理'?'fw-color':'tg-color'">{{item.czy}}</span>
...
export default {
  ...
  methods:{
      ...
      showDetail(index){
          ...
          this.$router.push({name:'emailDetails',query:{Id:_this.newsData[index].Id}})
      },
    ...
    var url = "/admin/appAgentAction!runAgent.action?agentId=Agent_SCHD_Index_Data_Leader&operateType=getMailbox&sqlTableName=View_ACT_RU_TASK&userId=";
