        ...
        <swipeout>
          <div v-for="(item, index) in qualityData" :key="item.Id">
            ...
            >
              <div
                slot="right-menu"
                v-if="item.lock == null && item.AddName == userId"
              >
                <swipeout-button
...
export default {
  data() {
    return {
      ...
      sqlTableName: 'U020_ZHGL_ZSK_ZXKS_KSFB'
    }
  },
  components: {
    Search,
  ...
  methods: {
    ...
    showExam(item) {
      ...
      if (Date.parse(new Date()) < Date.parse(item.kssj)) {
        _this.util.alert('考试还未开始')
        return
      } else if (Date.parse(new Date()) > Date.parse(item.jssj)) {
        _this.util.alert('考试已经结束')
        return
      }
      _this.util.post(
        '/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_Examination&appId=WeChat&Type=getIfThere',
        { id: item.Id },
        function(res) {
          if (res.data.data.length !== 0) {
            _this.util.alert('您已参加考试')
            return
          }
          _this.$router.push({
            name: 'examination',
      ...
      // this.util.post(
      //   '/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_Examination&appId=WeChat&Type=getpost',
      //   {},
      //   function(res) {
      //     if (res.status) {
      //       if (
      //         res.data.data[0] == undefined ||
      //         item.ckry.indexOf(res.data.data[0].xgwzw) == -1
      //       ) {
      //         _this.util.alert('您不在考试人员中')
      //         return
      //       } else if (Date.parse(new Date()) < Date.parse(item.kssj)) {
      //         _this.util.alert('考试还未开始')
      //         return
      //       } else if (Date.parse(new Date()) > Date.parse(item.jssj)) {
      //         _this.util.alert('考试已经结束')
    ...
    querryData(type, start) {
      ...
      this.util.post(
        '/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_Examination&appId=WeChat&Type=gettestQueries',
        params,
        function(res) {
          // console.log('1', res)
          _this.qualityData =
