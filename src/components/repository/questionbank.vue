<template>
  <div>
    ...
    <scroller lock-x scrollbar-y use-pullup use-pulldown height="-62" 
            @on-pullup-loading="loadMore" @on-pulldown-loading="refresh" 
            v-model="status" ref="scroller"
            :pullup-config="pullup" :pulldown-config="pulldown" >
      <div class="box2">
        <swipeout>
            ...
            <div slot="right-menu"  v-if="val.lock == null && val.AddName == userId">
              <!-- <swipeout-button  @click.native="onButtonClick('fav')" type="primary">编辑</swipeout-button> -->
              <swipeout-button @click.native="onButtonClick('delete',val.Id)" type="warn">删除</swipeout-button>
            </div>
            <div slot="content" class="demo-content vux-1px-t" @click="edit(val.Id)">
                    <div class="report" @click.stop="report(val.Id,val.xmid,val.AddName)">上报</div>
                    <div class="infos">
                      <div class="proName">
                        {{val.projectName}}
                      </div>
                    ...
                    <div class="status">
                      <div v-if="val.lock==null || val.lock==0" class="status-line bg-red">未审</div>
                      <div v-if="val.lock==-1" class="status-line bg-blue">送审</div>
                      <div v-if="val.lock==-5" class="status-line bg-green">已审</div>
                    </div>
                    </div>
          </swipeout-item>
                  </div>
...
<!--     <group>
    ...
    <div style="position: fixed;margin: auto;bottom: 20px;text-align: right; width: 100%;">
        <x-icon type="ios-plus-outline" @click="addData()" style="margin-right:0.3rem" size="60"></x-icon>
    </div>
  </div>
</template>
