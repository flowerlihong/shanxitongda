            ...
            <swipeout>
                <div v-for="(item,index) in qualityData" :key="item.Id">
                    <swipeout-item @on-close="handleEvents('on-close')" @on-open="handleEvents('on-open')" transition-mode="follow">
                        <div slot="right-menu" v-if="item.lock==null && item.AddName == userId">
                            ...
                            <div class="card facebook-card animated fadeInUp" @click="showDetail(index)">
                                ...
                                <div class="card-header no-border">
                                    ...
                                    <div class="facebook-date head-ft16">
                                        <span class=" ft14" style="color:#8994d2">{{item.projectName == undefined?'':(item.projectName.length>19?item.projectName.substr(0,19)+'...':item.projectName)}}</span>
                                    </div>
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            检查内容:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.inspectionContent == undefined?'':(item.inspectionContent.length>35?item.inspectionContent.substr(0,35)+'...':item.inspectionContent)}}
                                            </span>                                           
                                        </span>                                      
                                    </div> 
                                    ...
                                    <div class="facebook-date">
                                        ...
                                        <span class=" c999 title-item infoItems-right" >
                                            检查结果:
                                            <span :class="{'color-content':true,'color-red':item.inspectionResult=='不合格'}">
...
deactivated(){
  ...
  methods:{
      ...
      showDetail(index){
          ...
          this.$router.push({name:'addQuality',query:{pType:'detail',Id:_this.qualityData[index].Id}})
      },
    ...
    //   var sql = "select a.* from U009_Quality_Inspection a left join U_Project b on a.xmid=b.Id where "+  
    //             "(a.AddName = '"+userId+"'  or  '"+userId+"' in (select UserId from ORG_User_Role "+
    //             "where RoleId in  ('admin','Role_Sys_Company_Admin','Role_0001_002')) ) "
        var sql="select a.* from U009_Quality_Inspection  a left join U_Project b on a.xmid=b.Id "+
                " where  (RIGHT('"+DepartId+"',6)  = RIGHT(a.subCompany,6) or '"+DepartId+
                "'  in(select DepartId from ORG_Depart where DepartType='dept')  or   CHARINDEX('"+DepartId+"',a.subCompany)>0) "     
      var params ={
      ...
      this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_WeChar_Data_Query&appId=WeChat',params,function(res){
        console.log(res)
        _this.qualityData=type ==1? _this.qualityData.concat(res.data):cloneObj(res.data);
    ...
    addBx(){
        this.$router.push({name:'addQuality',query:{pType:'addNew',Id:''}})
    }
...
.infoItems-left{
    font-size: 9pt;width:50%;float: left;
}
.infoItems-right{
    font-size: 9pt;width:47%;text-align: right;
}
.color-content{
