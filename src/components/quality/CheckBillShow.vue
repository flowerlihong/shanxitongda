	   ...
	   <div class="approvalMiddle" style="text-align:left;">
	    ...
	    <template>
	  	...
	  	<div class="middle" :style="left">
			...
			<div v-for="(item,key) in checkBillDet" :key="key">
				<div class="piaojv">
...
import {getArgFromHref} from '@/utils/utils.js'
export default {
  name: 'approval',
  components:{
      // 名字
      Checker, CheckerItem,XButton
  },
  data () {
    return {
  ...
  methods: {
		...
		saveImprove(){
			...
			this.$router.push({name:'addImproveBill',query:{checkId:this.id}});

		},
		editData(type){
			this.$router.push({name:'addQualityInspect',query:{id:this.id}});
		}
