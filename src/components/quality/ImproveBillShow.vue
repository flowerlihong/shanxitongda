	...
	<div style="text-align:left;">
		...
		<div v-for="(item,index) in improveBillDet" :key="index">
			...
			<div v-if="item.imgs!=undefined&&item.imgs.length>0" class="piaojv" style="border-top:1px solid #eee">
				<span>附件</span>
				<p>
					<img v-for="(img,key) in item.imgs" :key="key" @click="previewImage(img.id,item.imgs)" style="width:50px;height:50px;margin:0 5px;" :src="webUrl+'/attachmentAction!look.action?id='+img.id+'&access_token='+access_token" alt="">
				</p>
			</div>
	...
	<div v-transfer-dom>
      <popup v-model="popupShow" position="right" width="100%">
		...
		<nav class="navbar navbar-default navbar-fixed-bottom">
			<p>
				<x-button @click.native="popupClose()" mini style="width:48%;height: 45px;" type="default">取消</x-button>
	   ...
	   loadData(){
		   ...
		   if(isNewFlag){
			   this.util.mask();
				var _this=this;
				var param = {
						operateType:'getQualityImproveDataByid',
						id:this.id
				};
	   ...
	   isInArray(arr,value){
		...
		saveReply(){
			...
			this.util.post("/admin/appDefaultFormAction!saveData.action",{mainFormData:JSON.stringify(mainData),filesData:JSON.stringify(filesData),deleteFilesData: _this.attachmentModel.deleteFilesList.join(","),
				sqlTableName:"T_QUALITY_CHECK_IMPROVE"},function(result){
				if(result.success){
			...
			// this.util.post("/mobile/wx/quality/saveReply",{mainFormData:JSON.stringify(mainData),filesData:JSON.stringify(filesData), deleteFilesData: _this.attachmentModel.deleteFilesList.join(","),
			// 	sqlTableName:"WX_QUALITY_CHECK_IMPROVE",replyType:replyType,checkId:_this.id},function(result){
			// 	_this.attachmentModel.fileList=new Array();
			// 	_this.attachmentModel.deleteFilesList=new Array();	
			// 	_this.replyContent="";
			// 	if(result.status){
			...
			// 		_this.popupShow = false;
			// 		_this.loadData();
			// 		_this.$store.state.ifRefreshQualityBillList=true;
			// 	}else{
			// 		_this.util.unmask()
			// 		_this.util.alert(result.msg);
			// 	}
		...
		updateBillStatus(replyType,checkId){
			var _this=this;
			var param = {
					operateType:'updateQualityBillStatus',
					replyType:replyType,
					checkId:checkId
			};
			this.util.post("/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_Inspect&appId=WeChat",param,function(res){
