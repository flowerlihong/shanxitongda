  ...
  <div class="visit">
  	...
  	<div class="visitCenter">
		...
		<div class="box" style="border-bottom: 1px solid #ccc;">
			...
			<checker id="inLine" type="checkbox" @on-change="onItemClick" v-model="improveType" default-item-class="demo4-item" selected-item-class="demo4-item-selected">
				<checker-item value="4">待整改</checker-item>
				<checker-item value="5">待复检</checker-item>
				<checker-item value="6">已通过</checker-item>
				<checker-item value="7">未通过</checker-item>
			</checker>
		</div>		
  ...
  methods: {
	...
	submit(){
		...
		if(this.visitPop.billType=="safety"){
			this.util.post('/mobile/wx/safety/searImproveBills',{selectedTabsStr:selectedTabsStr,
				improveType:this.improveType,userUid:this.userUid},function(result){
				_this.util.unmask();
				_this.visitPop.improveBillDatas = result;
			});
		}else{
			this.util.post('/mobile/wx/quality/searImproveBills',{selectedTabsStr:selectedTabsStr,
				improveType:this.improveType,userUid:this.userUid},function(result){
				_this.util.unmask();
				_this.visitPop.improveBillDatas = result;
			});
