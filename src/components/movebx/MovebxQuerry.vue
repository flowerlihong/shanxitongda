<template>
    <div class="choose">
        ...
        <div >            <!-- class="vux-1px-t" -->
            <swipeout>
                <div v-for="(item,index) in bxData" :key="item.Id">
                        ...
                        <div slot="content" class="demo-content noPadding">
                            <div class="card facebook-card animated fadeInUp" @click="showDetail(item)">
                                ...
                                <div class="card-header no-border">
                                    ...
                                    <div class="facebook-date head-ft16"><!-- {{item.projectName == undefined?'':(item.projectName.length>19?item.projectName.substr(0,19)+'...':item.projectName)}}-->
                                        <span class=" ft14 textOmit" style="color:#8994d2;width:calc(100% - 32px);">{{item.projectName}}</span>
                                    </div>

                                    <div class="facebook-date">
                                        <span class=" c999 title-item infoItems-left" > 报销日期: <span class="color-content">{{item.applyDate}}</span> </span>
                                        <span class=" c999 title-item infoItems-right" >报销金额: <span class="color-content">{{item.applyTotalMoney}}</span> </span>
                                    </div>
                                <div class="facebook-date">

                                        <span class=" c999 title-item infoItems-left" >余未核销借款:<span class="color-content">{{item.checkTotalApplyMoney}}</span></span>
                                        <span class=" c999 title-item infoItems-right" >实际支付:<span class="color-content">{{item.realGiveMoney}}</span></span>

                                    </div>
                                    <div class="facebook-date">
                                        <span class=" c999 title-item infoItems-left" style="width:60%;" >报销事由:
                                            <span class="color-content textOmit" style="vertical-align:bottom;width:calc(100% - 55px);" >
                                                <!-- {{item.Remark == undefined?'':item.Remark>9?(item.Remark.substr(0,9)+'...'):item.Remark}} -->
                                                {{item.Remark == undefined?'':item.Remark}}
                                            </span>
                                        </span>
                                        <span class=" c999 title-item infoItems-right" style="width:40%;">报销人:<span class="color-content">{{item.AddName_CN}}</span></span>
...
import { cloneObj, util } from "@/utils/utils.js";
export default {
  data() {
    return {
      search: "",
  ...
  methods: {
    ...
    showDetail(item) {
      // this.$router.push({name:'movebx3',query:{pType:'addNew',Id:Id}});
      var isReadOnly = item.lock == null ? "false" : "true";
      util.gotoByName("movebx3", {
        pType: "show",
        Id: item.Id,
        id: item.Id,
        _procInstId: item.ProcInsId,
    ...
    querryData(type, start) {
      this.util.mask();
      //  var obj = new Object();
      //obj['AddName'] = window.session.user.userId;
      // obj['projectNumber'] =
      var _this = this;
      var searchParams = {
      ...
      var sql =
        "select t.*,isnull(u.projectName,'') projectName,isnull(u.projectNum,'') projectNumber," +
        "o.UserName applyUserName,d.DepartName applyDepartName," +
        "isnull(a.accountName,'') accountName,a.account,a.accountType from U012_Expense_Account t " +
        "left join U_Project u on t.xmid = u.Id " +
        "left join ORG_User o on t.applyUserId = o.UserId " +
        "left join ORG_Depart d on t.applyDepartUid = d.Id " +
        "left join U012_Account_Manager a on t.accountId = a.Id where t.AddName='"+this.sessionUtil.getUserId()+"'";
      ...
      this.util.post(
        ...
        function(res) {
          ...
          _this.bxData =
            type == 1 ? _this.bxData.concat(res.data) : cloneObj(res.data);
    ...
    addBx() {
      // this.$router.push('movebx3')
      // this.$router.push({name:'movebx3',query:{pType:'addNew',Id:''}})
      this.util.gotoByName("movebx3", { pType: "addNew", Id: '', });
    }
  }
};
</script>
