          ...
          <div v-for="(item, index) in qualityData" :key="item.Id">
            ...
            >
              <!-- <div slot="right-menu" v-if="item.lock==null && item.AddName == userId">
                            <swipeout-button @click.native="onButtonClick('delete',item.zdId,index)" type="warn">{{('删除')}}</swipeout-button>
                        </div> -->
  ...
  methods: {
    ...
    showDetail(index, item) {
      ...
      this.$router.push({
        ...
        query: {
          pType: 'detail',
          zdId: _this.qualityData[index].zdId,
          data: item
        },
        params: { data: item }
      })
    ...
    querryData(type, start) {
      ...
      this.util.post(
        '/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_xaxi&appId=WeChat&Type=qualityInform',
        params,
        function(res) {
          console.log('1', res)
          _this.qualityData =
