                ...
                <div v-for="(item,index) in qualityData" :key="item.Id">
                    <swipeout-item @on-close="handleEvents('on-close')" @on-open="handleEvents('on-open')" transition-mode="follow">
                        ...
                        <div slot="content" class="demo-content noPadding">
                            <div class="card facebook-card animated fadeInUp" @click="showDetail(index,item)">
                                ...
                                <div class="card-header no-border">
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            立项名称:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.lxmc == undefined?'':(item.lxmc.length>35?item.lxmc.substr(0,35)+'...':item.lxmc)}}
                                            </span>                                           
                                        </span>                                      
                                    </div> 
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            站点名称:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.zdmc == undefined?'':(item.zdmc.length>35?item.zdmc.substr(0,35)+'...':item.zdmc)}}
                                            </span>                                           
                                        </span>                                      
                                    </div> 
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            订单名称:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.ddgcmc == undefined?'':(item.ddgcmc.length>35?item.ddgcmc.substr(0,35)+'...':item.ddgcmc)}}
                                            </span>                                           
                                        </span>                                      
                                    </div> 
                                    <div class="facebook-date">
                                        <span class=" c999 title-item "> 
                                            检查内容:
                                            <span class="color-content" style="display:block;float:right">                                                
                                                {{item.jcnr == undefined?'':(item.jcnr.length>35?item.jcnr.substr(0,35)+'...':item.jcnr)}}
                                            </span>                                           
                                        </span>                                      
                                    </div> 
                                    ...
                                    <div class="facebook-date">
                                        <span class=" c999 title-item infoItems-left" >
                                            ...
                                            <span class="color-content">
                                                {{item.DocCreated == undefined?'':(item.DocCreated.length>10?item.DocCreated.substr(0,11):item.DocCreated)}}
                                            </span>
                                        </span>
                                        <span class=" c999 title-item infoItems-right" >
                                            是否需要整改:
                                            <span :class="{'color-content':true,'color-red':item.sfxyzg=='不合格'}">
                                                {{item.sfxyzg}}
                                            </span>
                                        </span>
                                    </div>                                    
...
export default {
  ...
  methods: {
    ...
    showDetail(index,item) {
      ...
      this.$router.push({
        name: "safetyCheckAdd",
				query: { pType: "detail", Id: _this.qualityData[index].Id },
				params: {data: item }
      });
    },
    ...
    querryData(type, start) {
      ...
      this.util.post(
        "/admin/appAgentAction!runAgent.action?agentId=Agent_Wechat_xaxi&appId=WeChat&Type=examine",
        params,
        function(res) {
          console.log(res);
          _this.qualityData =
