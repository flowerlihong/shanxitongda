<template>
    <div class="choose">
        ...
        <div >            <!-- class="vux-1px-t" -->
            <swipeout>
                <div v-for="(item,index) in progressData" :key="item.Id">
...
export default {
  ...
  methods:{
      ...
      showDetail(index){
          ...
          this.$router.push({name:'addProgress',query:{pType:'detail',Id:_this.progressData[index].Id}})
      },
      ...
      //alert('on button click ' + type)
        ...
        if(type=='delete'){
            ...
            this.util.post(url,params,function(res){
                ...
                _this.progressData.splice(index,1)
            })
        }
      },
      handleEvents (type) {
    ...
    querryData(type,start){
     ...
     // obj['projectNumber'] = 
      ...
      var sql = "select a.* from U008_Graphic_Progress a where "+  
      ...
      this.util.post('/admin/appAgentAction!runAgent.action?agentId=Agent_WeChat_WeChar_Data_Query&appId=WeChat',params,function(res){
        console.log(res)
        _this.progressData=type ==1? _this.progressData.concat(res.data):cloneObj(res.data);
    ...
    addBx(){
        this.$router.push({name:'addProgress',query:{pType:'addNew',Id:''}})
    }
